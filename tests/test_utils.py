...
def test_empty_wrap(empty_text):
    expected = []
    wrapped = tablefmt._text_wrap(empty_text)
    assert expected == wrapped

def test_no_wrap(sample_text):
    expected = [sample_text]
    wrapped = tablefmt._text_wrap(sample_text)
    assert expected == wrapped

def test_simple_wrap(sample_text):
    expected = ['Foo', 'Bar']
    wrapped = tablefmt._text_wrap(sample_text, width=3)
    assert expected == wrapped

def test_double_wrap(long_text):
    expected = ['Foo', 'Bar', 'Baz']
    wrapped = tablefmt._text_wrap(long_text, width=3)
    assert expected == wrapped

def test_multiline_no_wrap(multiline_text):
    expected = ['FooBar Baz']
    wrapped = tablefmt._text_wrap(multiline_text)
    assert expected == wrapped

def test_multiline_with_wrap(multiline_text):
    expected = ['Foo', 'Bar', 'Baz']
    wrapped = tablefmt._text_wrap(multiline_text, width=3)
    assert expected == wrapped

def test_trailing_whitespace_wrap(whitespace_text):
    expected = ['FooBar']
    wrapped = tablefmt._text_wrap(whitespace_text)
    assert expected == wrapped
...
def test_empty_translation(empty_text):
    expected = empty_text
    translated = tablefmt._translate_tabs(empty_text)
    assert expected == translated

def test_empty_width(empty_text):
    assert tablefmt._printable_width(empty_text) == len(empty_text)

def test_translation_no_tabs(sample_text):
    expected = sample_text
    translated = tablefmt._translate_tabs(sample_text)
    assert expected == translated
...
def test_translation_with_tabs(tabbed_text):
    expected = 'Foo Bar'
    translated = tablefmt._translate_tabs(tabbed_text)
    assert expected == translated
    assert expected != tabbed_text

def test_width_with_tabs(tabbed_text):
    expected = 'Foo Bar'
    assert tablefmt._printable_width(tabbed_text) == len(expected)
